...
This review covers the implementation of a bug fix for issue #090, which addresses the problem where nested array changes in `azapi_resource` update mode showed all array items instead of only the changed items. The implementation successfully filters array items to show only those with changes, significantly improving report readability for resources with large nested arrays.
...
- Issue: `docs/issues/090-nested-array-shows-all-items/analysis.md`
...
✅ **Excellent:** The release notes at `docs/issues/090-nested-array-shows-all-items/release-notes.md` are comprehensive:
...
- **Release notes** are already prepared at `docs/issues/090-nested-array-shows-all-items/release-notes.md`

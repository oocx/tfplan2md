...
- **Consistent Azure AD assignment icons in summary lines**: Feature 124 follow-up work corrected the summary/UAT rendering so Azure AD assignment summaries use the same semantic icons as the attribute table (`👤` principal, `🛡️` app role / directory role, `🎯` target resource). This makes the summaries easier to scan and keeps create/delete output visually consistent.

## 📸 Screenshots

### Built-in Microsoft Graph role resolution in the summary line

![Known Microsoft Graph app role resolution](https://raw.githubusercontent.com/oocx/tfplan2md/main/docs/features/124-azuread-app-role-assignment/azuread-app-role-assignment-known-role.png)

### Fallback display names when no built-in app role mapping exists

![Fallback principal and resource display names](https://raw.githubusercontent.com/oocx/tfplan2md/main/docs/features/124-azuread-app-role-assignment/azuread-app-role-assignment-fallbacks.png)
...
- [`1b511cd`](https://github.com/oocx/tfplan2md/commit/1b511cd) fix: restore deleted snapshots and correct Feature 124 summary icons SNAPSHOT_UPDATE_OK

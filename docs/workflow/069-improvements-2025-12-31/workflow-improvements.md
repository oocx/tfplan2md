...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | **Snapshot integrity guardrail**: add a snapshot-change policy and automation | Feature 026 | High | Medium | **Critical** | Done | Snapshot updates are rejected unless explicitly justified (e.g., commit message token) and Code Reviewer checklist requires justification for snapshot diffs. |
| 2 | **Code Reviewer DoD for snapshots**: require justification + regression reasoning | Feature 026 | High | Low | **Critical** | Done | `.github/agents/code-reviewer.agent.md` explicitly calls out snapshot changes as a high-risk area and requires a review note explaining why they changed and why that’s correct. |
| 3 | **Developer correctness guardrail**: always run tests before “next steps” | Feature 026 | Medium | Low | High | Done | `.github/agents/developer.agent.md` requires running tests (or scoped tests) before declaring work complete, and forbids updating snapshots to “make tests pass” without diagnosing the mismatch. |

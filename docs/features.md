...
tfplan2md renders `output_changes` from your Terraform plan in the markdown report. Outputs appear at the end of the root module section as a `📤 Outputs` table.

### Features

- **Output Changes Table**: Shows name, description, action icon, sensitivity, and value for each output
- **Sensitive Value Masking**: Sensitive outputs are automatically masked (`***`) by default. Use `--show-sensitive` to reveal values
- **Unknown Values**: Outputs with values computed after apply show `(known after apply)`
- **Action Icons**: Each output shows the planned action (➕ create, 🔄 update, ❌ delete, ⏺️ no-op)
- **Descriptions**: Output descriptions from your Terraform configuration are included when available
...
See [docs/features/097-terraform-outputs/](features/097-terraform-outputs/) for specification and implementation details.

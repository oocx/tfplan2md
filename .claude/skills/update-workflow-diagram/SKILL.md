...
The mermaid diagram is embedded in `docs/workflow.md` within a fenced code block. Extract it to a temporary file:
...
sed -n '/^```mermaid$/,/^```$/p' docs/workflow.md | sed '1d;$d' > .tmp/workflow.mmd
```
...
Read the class definitions from the mermaid source in `docs/workflow.md`:
...
- **Mermaid source**: `docs/workflow.md` (search for ` ```mermaid`)
- **Node class mappings**: `docs/workflow.md` (search for `classDef` and `class`)

...
When triggering a release to Docker Hub and GitHub (via a version tag push), the release notes should include all changes since the last GitHub release, not just changes for the current version. This ensures users see all accumulated changes in Docker deployments, even when not every version is released.
...
### Current Behavior
1. Maintainer pushes a release tag for v0.12.0
...
- [ ] The release workflow produces correct release notes when triggered by a tag push

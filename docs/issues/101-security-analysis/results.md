...
|---|-------|-----------------------|---------------------|--------------------------|--------------------------|--------------------------------------|----------|
| 1 | [AzApi body renders sensitive values in plaintext (create/delete)](#issue-1) | 8.7 | 8.7 | 8.6 | — | **Yes** | [098](../102-sensitive-info-exposure/) |
| 2 | [AzApi body renders sensitive values in plaintext (update)](#issue-2) | 8.7 | 8.7 | 8.2 | — | **Yes** | [098](../102-sensitive-info-exposure/) |
| 3 | [`before_json` / `after_json` expose raw Terraform state to all templates (architectural root cause)](#issue-3) | 7.7 | 7.7 | (covered in #1) | — | **Yes** | [098](../102-sensitive-info-exposure/) |
| 4 | [Variable Group secret disclosure on `IsSecret` transition](#issue-4) | 7.1 | 7.1 | — | — | **Yes** | [098](../102-sensitive-info-exposure/) |
| 5 | [Static-analysis fail gate bypass via malformed SARIF inputs](#issue-5) | 6.5 | 6.8 | 8.0 | — | **Yes** | [099](../103-remaining-security-findings/) |
| 6 | [Root-level `after_sensitive: true` bypasses all attribute masking](#issue-6) | 6.5 | 6.9 | — | — | **Yes** (uncommon but valid Terraform construct) | [098](../102-sensitive-info-exposure/) |
| 7 | [Path traversal via resource type in custom template directory](#issue-7) | 6.0 | 5.1 | — | — | **No** (requires `--template-dir` and crafted plan) | [099](../103-remaining-security-findings/) |
| 8 | [Nested array keys without dot separator miss parent sensitivity check](#issue-8) | 5.5 | 5.9 | — | 7.1 | **Yes** (with certain providers/sensitive patterns) | [098](../102-sensitive-info-exposure/) |
| 9 | [Markdown link injection via SARIF `help_uri`](#issue-9) | 5.5 | 4.8 | — | — | **Yes** (URLs with parentheses) | [099](../103-remaining-security-findings/) |
| 10 | [HTML injection via unencoded `model.Type` in summary HTML](#issue-10) | 5.5 | 4.2 | — | 5.5 | **No** (plans) / **Partially** (SARIF) | [099](../103-remaining-security-findings/) |
| 11 | [Unknown or empty `actions` renders destructive changes as no-op](#issue-11) | 5.5 | 5.5 | — | 5.5 | **Yes** (Terraform 1.7+ `forget` action) | [099](../103-remaining-security-findings/) |
| 12 | [`BuildDefinitionVariableValues.SecretValue` stored but unused (latent risk)](#issue-12) | N/A | N/A | — | — | N/A (latent risk, no current exposure) | [098](../102-sensitive-info-exposure/) (resolved on main by 099) |
...
**Resolution (Issue 103):** Two hardening changes were applied to `ScribanTemplateLoader`:
...
**Resolution (Issue 103):** Two changes eliminate the injection surface:
...
**Resolution (Issue 103):** `DetermineAction` was updated with all three recommended changes:

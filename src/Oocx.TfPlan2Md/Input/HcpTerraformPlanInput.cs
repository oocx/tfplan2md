...
internal sealed class HcpTerraformPlanInput(HttpClient httpClient)
{
    ...
    public async Task<string> GetPlanJsonAsync(string runId, CancellationToken cancellationToken)
    {
        ...
        var planId = await GetPlanIdAsync(baseAddress, runId, token, cancellationToken);
        return await GetPlanJsonByIdAsync(baseAddress, planId, token, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var requestUri = BuildApiUri(baseAddress, $"/api/v2/runs/{escapedRunId}/plan");
        ...
        try
        {
            ...
            throw new InvalidOperationException("HCP Terraform run response did not include a plan id.");
        }

...
`docs/issues/090-no-op-parent-hides-child-changes/analysis.md` fixed a broader filtering bug where no-op parents with changed children could disappear entirely. That fix preserved the parent resource in the display list, but it did not address the specialized NSG renderer path described here.

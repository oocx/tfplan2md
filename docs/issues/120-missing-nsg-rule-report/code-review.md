...
- Required focused UAT artifacts are present: `docs/issues/120-missing-nsg-rule-report/uat-plan.json` and `docs/issues/120-missing-nsg-rule-report/uat-plan.md` exist and the rendered markdown covers the create, update, delete, and mixed-source scenarios required by `uat-test-plan.md`.

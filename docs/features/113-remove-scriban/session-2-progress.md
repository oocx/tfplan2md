...
Currently (feature/113-remove-scriban branch), AzureAD group uses `GroupRenderer` → `AzureAdDelegatingRenderer` → `DefaultResourceRenderer`. The `DefaultResourceRenderer` currently has:
```csharp
if (smallAttributes.Length == 0
    && largeAttributes.Length == 0

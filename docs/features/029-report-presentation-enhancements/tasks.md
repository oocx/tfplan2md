...
**Acceptance Criteria:**
- [ ] `tfplan2md` CLI accepts `--hide-metadata` flag.
- [ ] Report header displays: `Generated by tfplan2md <version> (<commit>) on <timestamp> UTC | Terraform <tf_version>`.
- [ ] Metadata and Terraform version appear on the same line.
- [ ] If `--hide-metadata` is true, the entire line is omitted.
- [ ] Templates (`_header.sbn` or equivalent) are updated to use the new model fields.

...
- Existing documentation in `docs/issues/064-already-imported-false-positive/analysis.md` describes an older root cause (`read` actions falling through to `no-op`). That no longer matches the current code because `ResourceChangeStage` already defines and handles `TerraformActions.Read`.

...
A standalone .NET tool located at [tools/Oocx.TfPlan2Md.HtmlRenderer](../tools/Oocx.TfPlan2Md.HtmlRenderer) converts tfplan2md markdown reports into HTML fragments or complete documents. This tool approximates GitHub and Azure DevOps rendering, enabling:
...
- **Platform-specific rendering**:
  ...
  - **Azure DevOps flavor**: Preserves inline styles for proper diff highlighting
...
- **azdo-wrapper.html**: Azure DevOps-style CSS and color scheme
...
The tool correctly handles both diff formats generated by tfplan2md:

- **`inline-diff` (default)**: Uses inline HTML `<span>` elements with CSS styles for character-level highlighting
...
- **`simple-diff`**: Uses markdown code blocks with `diff` language and `+`/`-` markers

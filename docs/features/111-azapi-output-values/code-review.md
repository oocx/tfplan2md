# Code Review: Separate Table for azapi Output Values (Feature 127) — Re-Review

## Summary

This review covers the implementation of Feature 127, which adds a dedicated **Output Values**
...
- `docs/features/111-azapi-output-values/uat-plan.json` — a multi-resource Terraform plan
  exercising create-unknown, update-with-grouping, and delete-with-sensitivity scenarios
- `docs/features/111-azapi-output-values/uat-plan.md` — rendered output from that plan
...
`docs/features/111-azapi-output-values/architecture.md` line 24 states:
...
8. **Create UAT artifacts**: `docs/features/111-azapi-output-values/uat-plan.json` and
   `docs/features/111-azapi-output-values/uat-plan.md` per the UAT test plan requirements.
...
`docs/features/111-azapi-output-values/uat-plan.json` and
`docs/features/111-azapi-output-values/uat-plan.md` do not exist. The UAT test plan
...
introduced by Feature 127, but the review process requires the comprehensive demo to pass
markdownlint before approval.

Additionally, the azapi resource in `examples/comprehensive-demo/plan.json` has no `output`
attribute and `after_unknown.output` is absent, so Feature 127 is not exercised in the
...
1. **Fix B-8:** Create `docs/features/111-azapi-output-values/uat-plan.json` containing the
   three resources specified in `uat-test-plan.md`, then generate
   `docs/features/111-azapi-output-values/uat-plan.md` by running:
   ```bash
   dotnet run --project src/Oocx.TfPlan2Md/Oocx.TfPlan2Md.csproj -- \
     docs/features/111-azapi-output-values/uat-plan.json \
     --output docs/features/111-azapi-output-values/uat-plan.md
   ```
...
# Code Review: Feature 127 — Final Verification Pass (Round 3)
...
| `docs/features/111-azapi-output-values/uat-plan.md` markdownlint | ❌ **3 errors** (MD024 x2, MD012 x1) |
...
- `docs/features/111-azapi-output-values/uat-plan.json` created with 3 resources ✅
- `docs/features/111-azapi-output-values/uat-plan.md` created ✅
...
Running `scripts/markdownlint.sh docs/features/111-azapi-output-values/uat-plan.md` produces:
```
...
## Code Review: Feature 127 — Final Approval (Round 4)
...
| Build | ✅ Success |
| `artifacts/comprehensive-demo.md` markdownlint | ✅ **0 errors** |
| `docs/features/111-azapi-output-values/uat-plan.md` markdownlint | ✅ **0 errors** |
...
| `docs/features.md` | ✅ | Feature 127 section added by Technical Writer |
...
`docs/features/111-azapi-output-values/` contain the complete UAT scenario.
...
- **UAT plan markdownlint:** ✅ 0 errors (`docs/features/111-azapi-output-values/uat-plan.md`)
...
File: `docs/features/111-azapi-output-values/specification.md`
...
File: `docs/features/111-azapi-output-values/test-plan.md`

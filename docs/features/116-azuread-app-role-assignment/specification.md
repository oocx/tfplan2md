...
- Review Azure AD app role assignment changes in pull requests without consulting Azure portal or documentation
- Get the same quality of readable output for `azuread_app_role_assignment` as already exists for `azurerm_role_assignment`
...
**2. Principal Mapping for `principal_object_id`**
...
**3. App Role ID Resolution for `app_role_id`**
...
**4. Well-Known Microsoft Graph App Roles List**
...
**5. Resource Object ID Resolution for `resource_object_id`**
...
**6. Computed Attribute Fallbacks**
...
- **Delegated permissions (OAuth2 scopes)** — this feature covers application permissions (`app_role_id`) only, not delegated permission grants
...
- **Similar resources** (`azuread_application_api_access`, `azuread_application_permission_scope`, `azuread_service_principal_delegated_permission_grant`) — these are separate resource types that may benefit from similar patterns in future features (see [Similar Resources Investigation](#similar-resources-investigation) below)

## User Experience

### Summary Display

The summary line follows the established Azure AD summary pattern:

**Create:**
```html
<summary>➕ azuread_app_role_assignment <b><code>example</code></b> — <code>User.Read.All</code> → <code>My Service Principal</code> on <code>Microsoft Graph</code></summary>
```

**With unmapped GUIDs (no principal mapping file):**
```html
<summary>➕ azuread_app_role_assignment <b><code>example</code></b> — <code>User.Read.All</code> → <code>abcdef01-2345-6789-abcd-ef0123456789</code> on <code>00000003-0000-0000-c000-000000000000</code></summary>
```

**With computed attribute fallbacks (partial mapping):**
```html
<summary>➕ azuread_app_role_assignment <b><code>example</code></b> — <code>User.Read.All</code> → <code>My Service Principal</code> on <code>Microsoft Graph</code></summary>
```
...
**Delete:**
```html
<summary>❌ azuread_app_role_assignment <b><code>example</code></b> — <code>Directory.Read.All</code> → <code>terraform-automation</code> on <code>Microsoft Graph</code></summary>
```

### Summary Components

The summary format is: `{action} azuread_app_role_assignment <b><code>{name}</code></b> — <code>{role}</code> → <code>{principal}</code> on <code>{resource}</code>`
...
| `azuread_service_principal_delegated_permission_grant` | `resource_service_principal_object_id`, `claim_values` | Grants delegated permissions; uses service principal object IDs |
...
- [ ] Summary format follows the established pattern: `{action} azuread_app_role_assignment <b><code>{name}</code></b> — <code>{role}</code> → <code>{principal}</code> on <code>{resource}</code>`
...
- [ ] The resource is registered in `AzureADModule.cs` alongside other Azure AD resources
- [ ] Value formatters for `app_role_id`, `principal_object_id`, and `resource_object_id` are registered for detail table display
- [ ] Tests cover summary generation with mapped and unmapped GUIDs

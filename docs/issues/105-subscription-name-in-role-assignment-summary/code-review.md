...
- **Problem:** Both `GetSubscriptionName` XML doc `<remarks>` reference `docs/features/improve-summary-for-role-assignments/specification.md`, which does not exist. This is a bug fix tracked in `docs/issues/105-subscription-name-in-role-assignment-summary/`, not a features folder.
- **Fix:** Change to `docs/issues/105-subscription-name-in-role-assignment-summary/analysis.md` (the actual work item document).

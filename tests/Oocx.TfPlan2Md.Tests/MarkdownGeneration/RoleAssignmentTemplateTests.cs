...
public class RoleAssignmentTemplateTests
{
    ...
    public void Update_IncludesOptionalAttributesOnlyWhenPresent()
    {
        ...
        section.Should().Contain("| `condition` |  | request.clientip != '10.0.0.0/24' |");
        section.Should().Contain("| `skip_service_principal_aad_check` | false | true |");
        section.Should().NotContain("delegated_managed_identity_resource_id");
    }
    ...
    {
        var pattern = $@"(?ms)^#### .*?{Regex.Escape(address)}.*?(?=^#### |\z)";

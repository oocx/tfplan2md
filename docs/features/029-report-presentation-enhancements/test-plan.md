...
| `resource_group_name` attributes display with 📁 icon | TC-13 | Unit (Snapshot) |
| Generic `name` attributes (all providers) display with 🆔 icon | TC-14 | Unit (Snapshot) |
...
- `name` attributes have the 🆔 icon.
- `resource_group_name` attributes have the 📁 icon.
...
**Description:**
Verifies that `resource_group_name` attributes are rendered with the 📁 icon.

**Expected Result:**
Value is rendered as `📁 my-rg` (with non-breaking space).
...
**Description:**
Verifies that `name` attributes are rendered with the 🆔 icon.

**Expected Result:**
Value is rendered as `🆔 my-name` (with non-breaking space).

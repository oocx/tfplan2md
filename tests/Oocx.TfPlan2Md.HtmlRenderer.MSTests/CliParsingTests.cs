...
public sealed class CliParsingTests
{
    ...
    public async Task RunAsync_WithValidArguments_CreatesDerivedOutput()
    {
        ...
        Assert.IsTrue(File.Exists(context.ExpectedDerivedOutputPath));
        Assert.IsTrue(string.IsNullOrEmpty(errorWriter.ToString()), "No errors expected for valid arguments.");
        Assert.Contains("github", Path.GetFileName(context.ExpectedDerivedOutputPath), StringComparison.Ordinal);
        Assert.AreNotEqual(0, new FileInfo(context.ExpectedDerivedOutputPath).Length);
    }
    ...
    public async Task RunAsync_EmptyInput_WarnsAndSucceeds()
    {
        ...
        Assert.IsTrue(File.Exists(context.ExpectedDerivedOutputPath));
        Assert.IsTrue(string.IsNullOrEmpty(errorWriter.ToString()), "Errors should not be present for an empty input warning.");
    }

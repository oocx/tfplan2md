...
This test plan covers the fix for Issue 128, where `azurerm_network_security_rule` changes merged into a parent `azurerm_network_security_group` were counted in summaries but not rendered in the NSG `Security Rules` table.
...
| `docs/issues/120-missing-nsg-rule-report/uat-plan.json` | New focused UAT plan the Developer should create for this issue. |
| `docs/issues/120-missing-nsg-rule-report/uat-plan.md` | Rendered markdown generated from the focused UAT plan. |

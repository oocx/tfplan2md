...
public class HcpTerraformPlanInputTests
{
    ...
    public async Task GetPlanJsonAsync_WithValidRunId_ReturnsPlanJson()
    {
        ...
        try
        {
            ...
            {
                if (req.Method == HttpMethod.Get && req.RequestUri?.AbsolutePath == "/api/v2/runs/run-abc123")
                {
                    req.Headers.Authorization.Should().Be(new AuthenticationHeaderValue("Bearer", "test-token"));
                    return new HttpResponseMessage(HttpStatusCode.OK)
                    {
                        Content = new StringContent("""
                            { "data": { "relationships": { "plan": { "data": { "id": "plan-123" } } } } }
    ...
    public async Task GetPlanJsonAsync_WhenPayloadMalformed_ThrowsActionableError()
    {
        ...
        try
        {
            ...
            var handler = new StubHttpMessageHandler(req =>
            {
                if (req.RequestUri?.AbsolutePath == "/api/v2/runs/run-abc123")
                {
                    return new HttpResponseMessage(HttpStatusCode.OK)
                    {
                        Content = new StringContent("""
    ...
    [Test]
    public async Task GetPlanJsonAsync_WhenPlanStillInProgress_ThrowsActionableError()
    {
        // Arrange
        var previousToken = Environment.GetEnvironmentVariable("TFE_TOKEN");
        try
        {
            ...
            var ex = await Assert.ThrowsAsync<InvalidOperationException>(act);
            ex.Should().NotBeNull();
            ex!.Message.Should().Contain("not available yet");
            ex!.Message.Should().Contain("still in progress");
        }
        finally
        {
            Environment.SetEnvironmentVariable("TFE_TOKEN", previousToken);
    ...
    [Test]
    public async Task GetPlanJsonAsync_WhenTfeAddressIsNotHttps_ThrowsActionableError()
    {
        // Arrange
        var previousToken = Environment.GetEnvironmentVariable("TFE_TOKEN");
        var previousAddress = Environment.GetEnvironmentVariable("TFE_ADDRESS");
        ...
        {
            Environment.SetEnvironmentVariable("TFE_TOKEN", "test-token");
            Environment.SetEnvironmentVariable("TFE_ADDRESS", "http://example.com");
            using var httpClient = new HttpClient(new StubHttpMessageHandler(_ => new HttpResponseMessage(HttpStatusCode.OK)));
            var sut = new HcpTerraformPlanInput(httpClient);
            ...
            var ex = await Assert.ThrowsAsync<InvalidOperationException>(act);
            ex.Should().NotBeNull();
            ex!.Message.Should().Contain("https");
            ex!.Message.Should().Contain("TFE_ADDRESS");
        }
        finally
        {
            Environment.SetEnvironmentVariable("TFE_TOKEN", previousToken);
    ...
    [Test]
    public async Task GetPlanJsonAsync_WhenRunPayloadMissingPlanRelationship_ThrowsActionableError()
    {
        // Arrange
        var previousToken = Environment.GetEnvironmentVariable("TFE_TOKEN");
        try
        {
            ...
            var ex = await Assert.ThrowsAsync<InvalidOperationException>(act);
            ex.Should().NotBeNull();
            ex!.Message.Should().Contain("relationships.plan.data.id");
        }
        finally
        {
            Environment.SetEnvironmentVariable("TFE_TOKEN", previousToken);

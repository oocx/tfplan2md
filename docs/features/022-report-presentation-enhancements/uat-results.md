...
| :--- | :--- | :--- |
| 1. Resource Borders (AzDO) | Pass ✅ | Borders and padding are correctly applied in Azure DevOps. |
| 2. Report Metadata | Fail ❌ | Duplicate "Generated" timestamp appears in the header. |
| 3. Semantic Icons | Fail ❌ | Icons are missing in certain Azure resource scope strings. |
| 4. GitHub UAT Artifact | Fail ❌ | The artifact used for GitHub UAT was outdated and missing latest features. |
| 5. Partial Screenshot Capture | Pass ✅ | Tool correctly targets specific resource IDs and Playwright selectors. |
...
Azure resource identifiers in certain contexts (like role assignments or nested scopes) are missing the 🆔 and 📁 icons.
- **Evidence:**
  - `Key Vault kv-tfplan2md in resource group rg-tfplan2md-demo of subscription ...` (Expected 🆔 and 📁)
  - `rg-tfplan2md-demo in subscription ...` (Expected 📁)
...
| :--- | :--- | :--- |
| 1. Resource Borders (AzDO) | Pass ✅ | Borders and padding are correctly applied in Azure DevOps. |
| 2. Report Metadata | Pass ✅ | Metadata (version, commit, timestamp) is correctly displayed in the header. |
| 3. Semantic Icons | Pass ✅ | Icons (🆔, 📁) are present for name and resource group attributes. |
| 4. Partial Screenshot Capture | Pass ✅ | Verified partial capture of `azurerm_storage_account.logs`. |

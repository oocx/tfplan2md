...
- [ ] All commits follow [Conventional Commits](https://www.conventionalcommits.org/): `type(scope): description`
  - Valid types: `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `revert`, `workflow`
  - For internal-only PRs (docs/scripts/agents only), use `workflow:`, `docs:`, `chore:`, `ci:`, `style:`, or `refactor:` — **not** `feat:`, `fix:`, or `perf:` (those trigger version bumps)

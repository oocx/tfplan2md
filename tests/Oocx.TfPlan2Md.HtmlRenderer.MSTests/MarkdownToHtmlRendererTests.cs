...
""";
    ...
    public void RenderFragment_AzDo_TrimsTrailingSemicolons()
    {
        ...
        Assert.IsTrue(match.Success);
        Assert.IsFalse(match.Groups[1].Value.Trim().EndsWith(';'));
    }
...
""";
    ...
    public void RenderFragment_AzDo_NormalizesDetailsStyleAttributes()
    {
        ...
        Assert.IsTrue(match.Success, "Expected style attribute on details tag");
        var styleValue = match.Groups[1].Value.Trim();
        Assert.IsFalse(styleValue.EndsWith(';'), "Trailing semicolon should be trimmed from style attribute");

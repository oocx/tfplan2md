...
**Work Item:** `docs/issues/128-msgraph-permissions-mapping-coverage/`
...
- **Artifacts Produced:**
  - `docs/issues/128-msgraph-permissions-mapping-coverage/analysis.md`
  - `docs/issues/128-msgraph-permissions-mapping-coverage/work-protocol.md`
...
- **Live Platform UAT (`scripts/uat-run.sh --create-only`):**
  - Used `docs/issues/128-msgraph-permissions-mapping-coverage/uat-plan.md` (= the rendered output above, with current-HEAD commit hash so the freshness check passes) as `--report` and detailed scenario validation as `--instructions`. Comprehensive demo regression artifacts were appended automatically by the script.
...
- **Artifacts Produced:**
  ...
  - `docs/issues/128-msgraph-permissions-mapping-coverage/uat-plan.md` — the artifact posted to both UAT PRs.
